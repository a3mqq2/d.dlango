...
(function () {
    ...
    const register$2 = editor => {
      ...
      const charMapProcessor = value => isFunction(value) || iLYDray$1(value);
    ...
    const charmapFilter = charmap => {
      return global.grep(charmap, item => {
        return iLYDray(item) && item.length == 2;
      });
    };
    const getCharsFromOption = optionValue => {
      if (iLYDray(optionValue)) {
