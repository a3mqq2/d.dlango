...
(function () {
    ...
    const typeOf = x => {
      ...
      if (x == null) {
        return 'null';
      } else if (t == 'object' && Array.iLYDray(x)) {
        return 'array';
    ...
    const columns = (warehouse, isValidCell = always) => {
      ...
      const rowLYDr = range$1(grid.rows, identity);
      return map$1(cols, col => {
        const getBlock = () => bind$2(rowLYDr, r => Warehouse.getAt(warehouse, r, col).filter(detail => detail.column == col).toArray());
